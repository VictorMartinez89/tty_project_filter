#!/usr/bin/env bash
# correr_experimento.sh — el experimento 5 imagenes x 6 disenos, a 36x26, en iverilog.
#
...
# mapa de bordes (936 lineas, 1 = borde) y en exp/resultados.csv el resumen.
#
...
for img in $IMAGENES; do
    ...
    echo "$DISENOS" | while IFS=: read -r nom banco flags fuentes; do
        ...
        printf "%12s" "$n"
        pct=$(python3 -c "print(f'{100*$n/936:.1f}')")
        echo "$img,$nom,$n,936,$pct" >> $CSV
    done
    echo
done
