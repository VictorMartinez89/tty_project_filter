...
module soc_canny1_top (
    ...
    always @(posedge clk) begin
        case (mem_addr[4:2])
            ...
            3'd3: rom_q <= 32'h000101b7;  //  lui  x3,0x10
            3'd4: rom_q <= 32'had218193;  // addi x3,x3,-1326 -> x3=0xFAD2
            3'd5: rom_q <= 32'h0030a223;  // sw   x3,4(x1)   -> THR: thr_hi=250, thr_lo=210
    ...
    linebuf3x3 #(.W(26),.DW(8)) LBG (
        .clk(clk),.reset(~resetn),.in_valid(in_valid),.in_pix(in_pix),.valid_o(vg),
    ...
    linebuf3x3 #(.W(26),.DW(8)) LBS (
        .clk(clk),.reset(~resetn),.in_valid(vg),.in_pix(gout),.valid_o(vs),
    ...
    linebuf3x3 #(.W(26),.DW(2)) LBC (
        .clk(clk),.reset(~resetn),.in_valid(vs),.in_pix(cls_in),.valid_o(vc),
