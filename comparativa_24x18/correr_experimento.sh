#!/usr/bin/env bash
# correr_experimento.sh — el experimento 5 imagenes x 6 disenos, a 24x18, en iverilog.
#
...
# mapa de bordes (432 lineas, 1 = borde) y en exp/resultados.csv el resumen.
#
...
for img in $IMAGENES; do
    ...
    echo "$DISENOS" | while IFS=: read -r nom banco flags fuentes; do
        ...
        printf "%12s" "$n"
        pct=$(python3 -c "print(f'{100*$n/432:.1f}')")
        echo "$img,$nom,$n,432,$pct" >> $CSV
    done
    echo
done
