#!/usr/bin/env python3
# mk_img_24x18.py — prepara las 5 imagenes de la tesis a 24 filas x 18 columnas
# para el experimento 5x6 en simulacion Verilog.
#
#   img/<nombre>.hex   -> 432 bytes en hexadecimal (uno por linea), orden raster
...
for nombre, path in FUENTES.items():
    ...
    with open(os.path.join(AQUI, "img", f"{nombre}.hex"), "w") as f:
        for fila in g:
            for px in fila:
                f.write(f"{px:02x}\n")
    Image.fromarray(g).resize((W * 18, H * 18), Image.NEAREST) \
