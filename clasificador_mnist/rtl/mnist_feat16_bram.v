...
module mnist_feat16_bram #(
    ...
    output wire [128*CW-1:0] cnt_o,        // 16 zonas x 8 orientaciones, empaquetados
    ...
    // puente: si el pixel anterior incremento ESTA misma casilla, el valor
    // leido ya es viejo. Sin esto se pierde una cuenta cada vez que dos
    // pixeles seguidos caen en la misma zona y orientacion, que pasa mucho.
    wire [CW-1:0] valor = (q_val && q_dir == dir) ? q_rd + 1'b1 : q_rd;
    integer i;
        ...
        if (reset || clr) begin
            ...
            if (vc && !listo) begin
                if (!arrancado) lat_cnt <= lat_cnt + 1'b1;   // tragarse la latencia
                    ...
                    end else if (q_val && q_rd != {CW{1'b1}}) cnt[q_dir] <= valor + 1'b1;
    ...
    genvar k;
    generate for (k = 0; k < 128; k = k + 1) begin : sal
