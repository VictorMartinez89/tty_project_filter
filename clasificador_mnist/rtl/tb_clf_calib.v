// tb_clf_calib.v — el clasificador con los sesgos calibrados, contador por contador.
//
...
module tb_clf_calib;
    ...
    reg [32*CW-1:0] cnt_i = 0;
    reg [10:0] n_bordes = 0;
    wire done, valido; wire [3:0] digito; wire signed [21:0] score;

    mnist_clf_canny_calib u (.clk(clk), .reset(reset), .start(start), .cnt_i(cnt_i),
        .n_bordes(n_bordes), .done(done), .digito(digito), .valido(valido), .score(score));

    always #5 clk = ~clk;

    integer i, fd, n;
    reg [31:0] v [0:32];          // 32 contadores + n_bordes
    ...
    initial begin
        ...
        $readmemh(fin, v);
        for (i = 0; i < 32; i = i + 1) cnt_i[i*CW +: CW] = v[i][CW-1:0];
        n_bordes = v[32][10:0];
