#!/usr/bin/env python3
"""verificar_top_canny_fw.py — la cadena del SoC y de los chips 13/14, contra el golden.
...
encadenan las imagenes sin pausa, porque lo que falla no es el primer cuadro.
  ...
  Uso:  python3 verificar_top_canny_fw.py [n_imagenes] [gap]
"""
...
for lo in range(0, N, LOTE):
    ...
    subprocess.run(["vvp", "tmp/topcfw.vvp", f"+IN=tmp/tc_{TAG}.hex",
...
print(f"    acierto {acc}/{vistos} = {acc/max(vistos,1):.2%}   (el .vh dice 92.46 %)")
