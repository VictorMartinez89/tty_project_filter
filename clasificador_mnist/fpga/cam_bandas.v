// cam_bandas.v — LOS 8 BITS DE cam_d, UNO POR BANDA, EN UNA SOLA PANTALLA.
//
//   La prueba de la rampa (cam_patron, FUENTE=1) salio LIMPIA: PCLK, el framebuffer y el
//   camino al display estan bien. Queda un solo sospechoso, el bus cam_d[7:0]. Este diseno
//   dice CUAL de los ocho cables falla, sin osciloscopio y en una sola corrida.
//
//   La pantalla se parte en 8 bandas horizontales de 10 filas. En la banda de arriba se pinta
//   el bit 7 de cam_d, en la siguiente el 6, y asi hasta el bit 0 abajo. Cada banda es blanca
//   donde ese bit vale 1 y negra donde vale 0.
//
//   COMO SE LEE, tapando y destapando el lente:
//     * banda que CAMBIA al tapar        -> ese bit llega bien
//     * banda toda BLANCA o toda NEGRA   -> ese bit esta PEGADO (cable suelto a VCC o GND)
//     * banda con ruido que NO responde  -> ese bit esta FLOTANDO (cable desconectado)
//   Los bits altos deberian responder mucho al tapar; los bajos siempre tienen algo de ruido.
// cam_bits.v — DIAGNOSTICO DEL BUS DE DATOS de la camara.
//
//   Es el cam_display.v de Victor con UNA linea cambiada: se pueden apagar bits de cam_d antes
//   de mostrarlos. Sirve para encontrar lineas flojas sin osciloscopio.
//
//   El sintoma que lo motiva: la pantalla muestra MOTEADO fino sobre un GRADIENTE real. El
//   gradiente prueba que la camara responde a la luz; el moteado, que algunos bits llegan mal.
//   Si unos pines de datos estan flojos, los bits altos que si llegan dan el brillo grueso
//   correcto y los flojos agregan ruido aleatorio encima. Eso es exactamente lo que se ve.
//
//   MASCARA = 8'hF0 -> se muestran solo los 4 bits ALTOS (los bajos se fuerzan a cero).
//     * si con la mascara la imagen se LIMPIA y aparece la escena, los bits BAJOS estan flojos.
//     * si sigue moteada, el problema esta en los bits altos, en PCLK o en HREF.
//   Es una biseccion del bus: cada corrida descarta la mitad.
...
module top (
    ...
    // fila actual dentro del framebuffer de 60x80, y de ahi la banda de 10 filas
    wire [6:0] fila_fb = waddr_wr / 7'd60;
    wire [2:0] banda   = fila_fb[6:3] > 4'd7 ? 3'd7 : fila_fb[6:3];   // 80 filas / 10 = 8 bandas
    reg        we      = 1'b0;
    reg [12:0] wadr    = 13'd0;
    reg [7:0]  wdat    = 8'd0;
