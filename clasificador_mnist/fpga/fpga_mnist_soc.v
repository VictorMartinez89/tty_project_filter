// fpga_mnist_soc.v — EL SoC COMPLETO EN SILICIO: RISC-V + clasificador de digitos.
...
module top #(
    ...
    soc_ctrl SOC (.clk(clk), .resetn(~reset), .thr_o(thr_cpu), .cpu_wrote(cpu_wrote));

    mnist_top #(.H(28),.W(28),.CW(9)) CLF (
        .clk(clk), .reset(reset), .clr(clr),
        .in_valid(feed), .in_pix(pix), .thr(thr_cpu),
