// mnist_cam78.v — ESCRIBI UN DIGITO Y LA FPGA TE DICE CUAL ES, con la cadena del 97,22 %.
...
//   La SCCB, la ventana, el framebuffer y el driver ILI9341 siguen VERBATIM: estan probados.
//
...
//   rojo=hubo que realinear un cuadro, azul=latido.
//
...
module top #(
    ...
    // ======== la cadena del 97,22 %: ventana -> realineo -> mnist_top78 ========
    //   El MISMO modulo que simula tb_cam78.v. Dice 0..9 o 10 (NADA, que el glifo pinta como raya).
    wire [3:0] dig_pclk; wire hubo; wire realineo;
    cam78_cadena CAD78 (
        .pclk(cam_pclk), .reset(~cfg_done),
        .w_valid(w_valid), .w_pix(w_pix), .w_fin(w_fin),
        .digito(dig_pclk), .hubo(hubo), .realineo(realineo));
    ...
    SB_RGBA_DRV #(
        ...
        .RGB0PWM(cfg_done), .RGB1PWM(realineo), .RGB2PWM(hb[23]),
