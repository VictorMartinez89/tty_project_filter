// cam_negedge.v — MUESTREAR cam_d EN EL FLANCO DE BAJADA de PCLK.
//
//   El diagnostico por UART dio AND=00 OR=FF: NINGUN bit esta pegado, los ocho cables funcionan.
//   Con el bus sano y la imagen en ruido, queda un solo hueco sin cubrir: el INSTANTE en que se
//   muestrea. La prueba de la rampa probo que el reloj corre, pero la rampa se genera adentro y
//   no depende del setup/hold de cam_d respecto de PCLK.
//
//   Si el dato de la camara CAMBIA justo en el flanco en que se lo captura, cada bit igual va a
//   valer 0 y 1 a lo largo del cuadro -por eso AND=00 OR=FF- pero el byte capturado es basura.
//   Ruido con todos los bits sanos: encaja exacto con lo que se ve.
//
//   Es un problema clasico del OV7670: segun el registro COM10 (bit 4, PCLK reverse) el dato es
//   valido en un flanco o en el otro, y los clones no siempre respetan el valor por defecto.
//
//   ESTE ARCHIVO ES EL cam_display.v DE VICTOR CON UNA SOLA LINEA CAMBIADA: el bloque que lee
//   la camara pasa de `posedge cam_pclk` a `negedge cam_pclk`. Si la imagen aparece, era esto.
...
module top (
    ...
    always @(negedge cam_pclk) begin        // <- LA UNICA DIFERENCIA
        href_d <= cam_href;
        we     <= 1'b0;
        if (~cam_href) begin
