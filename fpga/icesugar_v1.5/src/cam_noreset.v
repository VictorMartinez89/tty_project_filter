// cam_noreset.v — Config SIN reset: solo COM7=0x00 (grises YUV). Mantiene vivo el
// PCLK del default de encendido (el reset lo apagaba).
//
// UART imprime cada ~0.35s:   "<PP> <H> <LL>"
//   PP = contador libre por flanco de PCLK -> si CAMBIA = PCLK vivo.
//   H  = 1 si HREF se vio en alto.
//   LL = brillo promedio (debe moverse con la luz; en YUV ~40+ aun oscuro).
//
//   ROJO=configurando, VERDE=config lista, AZUL=heartbeat.
//   Tabla: COM7=0x00 (sin 0x80 reset). Fin.
...
module top (
    ...
    // ---- tabla SIN reset: solo formato ----
    reg [15:0] rom;
    reg [4:0]  idx = 5'd0;
    always @(*) case (idx)
        5'd0:  rom = 16'h12_00;   // COM7 = YUV (grises), SIN reset previo
    ...
    always @(posedge clk) if (tick && boot_ok && !cfg_done) begin
        ...
        OP_STOP: begin
            case (ph)
                ...
                2'd3: begin pc <= pc + 1'b1; dly <= 16'd99; end
            endcase
            ph <= ph + 1'b1;
        end
    ...
    // ---- brillo ----
    reg [11:0] acc = 12'd0;
    always @(posedge cam_pclk) if (cam_href) acc <= acc - (acc >> 4) + {4'd0, cam_d};
    wire [7:0] lum = acc[11:4];
    reg [7:0] lum_s0 = 0, lum_s1 = 0;
    ...
    always @(posedge clk) begin
        tx_start <= 1'b0;
        case (pst)
            4'd0: if (ptick && cfg_done) begin pp <= pcnt_s1; ll <= lum_s1; pst <= 4'd1; end
