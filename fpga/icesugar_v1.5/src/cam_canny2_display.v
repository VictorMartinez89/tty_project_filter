// cam_canny2_display.v — CANNY Etapa 2a: Gaussian 3x3 -> Sobel 3x3 -> doble umbral
//                        -> HISTERESIS DE 1 SALTO -> pantalla.
//
// NUEVO vs cam_canny_display.v (Etapa 1):
//   Antes se mandaba la CLASE cruda a la pantalla (fuerte=blanco, debil=gris, fondo=negro),
//   asi que el ruido debil aislado se veia como manchitas grises.
//   Ahora la clase pasa por un TERCER juego de line buffers + ventana 3x3, y se aplica
//   HISTERESIS DE 1 SALTO sobre el pixel CENTRAL:
//       borde = fuerte                        -> blanco
//             | (debil AND algun vecino-8 fuerte) -> blanco   (se "rescata" el debil conectado)
//             | resto                         -> negro        (el debil aislado se descarta)
//   => bordes mas limpios y conectados, sin manchas grises sueltas.
//
// COSTO: la clase son 2 bits (no 8), asi que los 2 line buffers nuevos son 2x60x2 = 240 FF
//        + 9 registros de ventana. Barato: no deberia comprometer los 5280 LC.
//
// NOTA: esta etapa agrega 1 fila + 1 columna de latencia => la imagen se corre ~61 px.
//       Si queda descuadrada, re-ajusta OFFSET (esta abajo, buscar "OFFSET").
//
// Pines (identicos a cam_canny_display / cam_display.pcf):
...
module top (
    ...
    // ==== submuestreo 60x80 + Gaussian -> Sobel -> clase -> histeresis (dominio pclk) ====
    reg href_d  = 1'b0;
    reg        parity  = 1'b0;
    reg [7:0]  curY    = 8'd0;
    ...
    // --- NUEVO: clase de 2 bits (doble umbral) ---
    //     2 = fuerte, 1 = debil, 0 = nada
    localparam [1:0] CL_NADA=2'd0, CL_DEBIL=2'd1, CL_FUERTE=2'd2;
    wire [1:0] cls_in = (mag > 8'd70) ? CL_FUERTE :
                        (mag > 8'd30) ? CL_DEBIL  : CL_NADA;

    // --- NUEVO: Etapa histeresis 1-salto: line buffers de CLASE (2 bits) + ventana 3x3 ---
    reg [1:0] cline1 [0:59];
    reg [1:0] cline2 [0:59];
    reg [1:0] c00=0,c01=0,c02=0, c10=0,c11=0,c12=0, c20=0,c21=0,c22=0;
    // el centro de la ventana de clase es c11
    ...
    // histeresis 1 salto: fuerte -> borde ; debil -> borde SOLO si toca un fuerte ; nada -> no
    wire edge_px = (c11==CL_FUERTE) ? 1'b1 :
                   (c11==CL_DEBIL)  ? any_strong : 1'b0;
        ...
        if (~cam_href) begin
            ...
            if (parity == 1'b0) curY <= cam_d;
                ...
                if (rowkeep==3'd0 && colkeep==4'd0 && fbx<7'd60) begin
                    ...
                    // --- NUEVO: Clase: SR + ventana (entrada = cls_in, 2 bits) ---
                    ...
                    // --- salida: histeresis de 1 salto (binaria: borde o fondo) ---
                    we <= 1'b1; wadr <= waddr_wr;
                    wdat <= edge_px ? 8'hFF : 8'h00;
    ...
    // OFFSET: encuadre. La etapa nueva agrega ~1 fila + 1 col de latencia (~61 px).
    // Si la imagen queda corrida, prueba 1800 (Etapa 1) o ajusta de a ~60.
