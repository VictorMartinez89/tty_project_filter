// cam_clkcheck.v — Tras configurar (reset+YUV), MIDE si PCLK y HREF siguen vivos.
// Distingue: ¿se freno el PCLK, o es el HREF el que no se activa?
//
// UART imprime cada ~0.35s:   "<PP> <H>"
//   PP = byte (hex) de un contador que avanza con CADA flanco de PCLK
//        (SIN filtrar por HREF).  -> si PP CAMBIA entre lineas = PCLK VIVO.
//                                    si PP queda fijo           = PCLK FRENADO.
//   H  = 1 si HREF se vio en alto alguna vez ; 0 si nunca.
//
//   ROJO=configurando, VERDE=config lista, AZUL=heartbeat.
//
// Config minima: COM7=0x80 (reset) -> pausa -> COM7=0x00 (YUV). Fin.
// Pines igual que cam_config3.
...
module top (
    ...
    reg [7:0] pch;
    always @(*) case (pst)
        4'd1: pch = hex(snap[7:4]);
        4'd2: pch = hex(snap[3:0]);
        4'd3: pch = " ";
        4'd4: pch = href_seen ? "1" : "0";
        4'd5: pch = 8'h0D;
        4'd6: pch = 8'h0A;
        default: pch = 8'h00;
    endcase
    always @(posedge clk) begin
        tx_start <= 1'b0;
        case (pst)
            4'd0: if (ptick && cfg_done) begin snap <= pcnt_s1; pst <= 4'd1; end
