// cam_config.v — Configura OV7670 (grises + QQVGA) por SCCB en iCESugar v1.5
// Paso 4b: al arrancar, escribe una tabla de registros por SCCB. Luego mide el
// brillo (luma Y) de la imagen ya configurada.
//
//   AZUL parpadea  = FPGA viva.
//   VERDE fijo      = tabla de configuracion enviada (camara configurada).
//   VERDE brillo PWM (cuando termina) sigue la luz de la escena, ahora rango full.
//   UART (pin 6): imprime el brillo en hex ~3/seg -> debe ir de 00 a FF.
//
// Pines: clk=35, cam_xclk=2, cam_scl(SIOC)=26, cam_sda(SIOD)=27,
//        cam_pclk=45, cam_href=3, cam_d[0..7]=48,46,44,43,38,34,31,42,
//        uart_tx=6, leds=39/40/41

module top (
    input  wire       clk,
    output wire       cam_xclk,
    output wire       cam_scl,
    inout  wire       cam_sda,
    input  wire       cam_pclk,
    input  wire       cam_href,
    input  wire [7:0] cam_d,
    output wire       uart_tx,
    output wire       led_r,
    output wire       led_g,
    output wire       led_b
);
    assign cam_xclk = clk;

    // ===================== SCCB master (solo escritura) =====================
    reg sda_oe = 1'b0;
    assign cam_sda = sda_oe ? 1'b0 : 1'bz;
    reg scl = 1'b1;
    assign cam_scl = scl;

    reg [5:0] tdiv = 6'd0;
    wire tick = (tdiv == 6'd29);
    always @(posedge clk) tdiv <= tick ? 6'd0 : tdiv + 1'b1;

    // ---- Tabla de registros {addr, valor}. 16'hFFFF = fin. ----
    // Grises (YUV, leemos Y) + QQVGA 160x120.
    reg [15:0] rom;
    reg [4:0]  idx = 5'd0;        // indice en la tabla
    always @(*) case (idx)
        5'd0:  rom = 16'h12_80;   // COM7: RESET total
        5'd1:  rom = 16'h12_00;   // COM7: salida YUV (de aqui sacamos luma)
        5'd2:  rom = 16'h0C_04;   // COM3: habilitar DCW/scaling
        5'd3:  rom = 16'h3E_1A;   // COM14: PCLK escalado, manual
        5'd4:  rom = 16'h70_3A;   // SCALING_XSC
        5'd5:  rom = 16'h71_35;   // SCALING_YSC
        5'd6:  rom = 16'h72_22;   // SCALING_DCWCTR: /4 en H y V -> QQVGA
        5'd7:  rom = 16'h73_F2;   // SCALING_PCLK_DIV: /4
        5'd8:  rom = 16'hA2_02;   // SCALING_PCLK_DELAY
        5'd9:  rom = 16'h11_01;   // CLKRC: divisor de reloj interno
        5'd10: rom = 16'h1E_00;   // MVFP: sin espejo/flip
        5'd11: rom = 16'hFF_FF;   // FIN
        default: rom = 16'hFF_FF;
    endcase

    wire        tbl_end = (rom == 16'hFF_FF);
    wire [7:0]  reg_addr = rom[15:8];
    wire [7:0]  reg_val  = rom[7:0];

    // ---- Secuencia de UNA escritura: START, 0x42, addr, val, STOP ----
    // pc: 0=START,1=WR 0x42,2=WR addr,3=WR val,4=STOP,5=next
    localparam OP_START=3'd0, OP_WR=3'd1, OP_STOP=3'd2, OP_NEXT=3'd3, OP_DONE=3'd4;
    reg [2:0] pc = 3'd0;
    reg [1:0] ph = 2'd0;
    reg [3:0] bi = 4'd0;
    reg       cfg_done = 1'b0;

    reg [2:0] optype;
    always @(*) case (pc)
        3'd0: optype = OP_START;
        3'd1: optype = OP_WR;
        3'd2: optype = OP_WR;
        3'd3: optype = OP_WR;
        3'd4: optype = OP_STOP;
        default: optype = OP_NEXT;
    endcase

    reg [7:0] wbyte;
    always @(*) case (pc)
        3'd1: wbyte = 8'h42;        // direccion de escritura
        3'd2: wbyte = reg_addr;     // registro
        3'd3: wbyte = reg_val;      // valor
        default: wbyte = 8'h00;
    endcase

    // arranque: esperar un toque tras reset antes de configurar
    reg [19:0] boot = 20'd0;
    wire boot_ok = &boot;
    always @(posedge clk) if (!boot_ok) boot <= boot + 1'b1;

    always @(posedge clk) if (tick && boot_ok && !cfg_done) begin
        case (optype)
        OP_START: begin
            case (ph)
                2'd0: begin sda_oe <= 1'b0; scl <= 1'b1; end
                2'd1: begin sda_oe <= 1'b1; scl <= 1'b1; end
                2'd2: begin scl <= 1'b0; end
                2'd3: begin pc <= pc + 1'b1; end
            endcase
            ph <= ph + 1'b1;
        end
        OP_WR: begin
            case (ph)
                2'd0: begin scl <= 1'b0;
                            if (bi < 4'd8) sda_oe <= ~wbyte[3'd7 - bi[2:0]];
                            else           sda_oe <= 1'b0; end
                2'd1: scl <= 1'b1;
                2'd2: scl <= 1'b1;
                2'd3: begin scl <= 1'b0;
                            if (bi == 4'd8) begin bi <= 4'd0; pc <= pc + 1'b1; end
                            else bi <= bi + 1'b1; end
            endcase
            ph <= ph + 1'b1;
        end
        OP_STOP: begin
            case (ph)
                2'd0: begin scl <= 1'b0; sda_oe <= 1'b1; end
                2'd1: begin scl <= 1'b1; sda_oe <= 1'b1; end
                2'd2: begin scl <= 1'b1; sda_oe <= 1'b0; end
                2'd3: pc <= pc + 1'b1;
            endcase
            ph <= ph + 1'b1;
        end
        OP_NEXT: begin
            if (tbl_end) cfg_done <= 1'b1;   // tabla terminada
            else begin idx <= idx + 1'b1; pc <= 3'd0; end
        end
        default: cfg_done <= 1'b1;
        endcase
    end

    // ===================== Medir brillo (luma) =====================
    // En YUV el byte de luma Y son los pixeles pares (Y U Y V Y U...).
    // Para un fotometro simple, promediamos TODOS los bytes igual; con grises
    // el rango ya se abre completo. (El Sobel del paso 5 tomara Y real.)
    reg [11:0] acc = 12'd0;
    always @(posedge cam_pclk)
        if (cam_href) acc <= acc - (acc >> 4) + {4'd0, cam_d};
    wire [7:0] lum = acc[11:4];

    reg [7:0] lum_s0 = 8'd0, lum_s1 = 8'd0;
    always @(posedge clk) begin lum_s0 <= lum; lum_s1 <= lum_s0; end

    // ===================== UART: imprime brillo hex =====================
    reg  [7:0] tx_data  = 8'd0;
    reg        tx_start = 1'b0;
    wire       tx_busy;
    uart_tx u_tx (.clk(clk), .start(tx_start), .data(tx_data), .tx(uart_tx), .busy(tx_busy));

    function [7:0] hex; input [3:0] n;
        hex = (n < 4'd10) ? (8'h30 + {4'd0,n}) : (8'h41 + {4'd0,n} - 8'd10);
    endfunction

    reg [21:0] ptmr = 22'd0;
    wire ptick = &ptmr;
    always @(posedge clk) ptmr <= ptick ? 22'd0 : ptmr + 1'b1;

    reg [2:0] pst = 3'd0;
    reg [7:0] val = 8'd0;
    always @(posedge clk) begin
        tx_start <= 1'b0;
        case (pst)
            3'd0: if (ptick && cfg_done) begin val <= lum_s1; pst <= 3'd1; end
            3'd1: if (!tx_busy && !tx_start) begin tx_data <= hex(val[7:4]); tx_start <= 1'b1; pst <= 3'd2; end
            3'd2: if (!tx_busy && !tx_start) begin tx_data <= hex(val[3:0]); tx_start <= 1'b1; pst <= 3'd3; end
            3'd3: if (!tx_busy && !tx_start) begin tx_data <= 8'h0D;         tx_start <= 1'b1; pst <= 3'd4; end
            3'd4: if (!tx_busy && !tx_start) begin tx_data <= 8'h0A;         tx_start <= 1'b1; pst <= 3'd0; end
            default: pst <= 3'd0;
        endcase
    end

    // ===================== LEDs =====================
    reg [7:0]  pwm = 8'd0;
    reg [23:0] hb  = 24'd0;
    always @(posedge clk) begin pwm <= pwm + 1'b1; hb <= hb + 1'b1; end
    // antes de configurar: verde apagado; ya configurado: verde sigue el brillo
    wire verde = cfg_done & (pwm < lum_s1);

    SB_RGBA_DRV #(
        .CURRENT_MODE("0b1"),
        .RGB0_CURRENT("0b000011"),
        .RGB1_CURRENT("0b000001"),
        .RGB2_CURRENT("0b000001")
    ) rgba (
        .CURREN(1'b1), .RGBLEDEN(1'b1),
        .RGB0PWM(verde),            // VERDE = configurada + brillo de escena
        .RGB1PWM(1'b0),
        .RGB2PWM(hb[23]),           // AZUL = heartbeat
        .RGB0(led_r), .RGB1(led_g), .RGB2(led_b)
    );
endmodule

// ================= UART TX 8N1 =================
module uart_tx #(
    parameter CLK_FREQ = 12_000_000,
    parameter BAUD     = 115_200
)(
    input  wire       clk,
    input  wire       start,
    input  wire [7:0] data,
    output wire       tx,
    output reg        busy
);
    localparam integer DIV = CLK_FREQ / BAUD;
    reg [12:0] clkcnt  = 13'd0;
    reg [3:0]  nbits   = 4'd0;
    reg [9:0]  shifter = 10'h3FF;
    assign tx = shifter[0];
    initial busy = 1'b0;
    always @(posedge clk) begin
        if (busy) begin
            if (clkcnt == DIV-1) begin
                clkcnt <= 13'd0;
                if (nbits == 4'd9) busy <= 1'b0;
                else begin shifter <= {1'b1, shifter[9:1]}; nbits <= nbits + 1'b1; end
            end else clkcnt <= clkcnt + 1'b1;
        end else if (start) begin
            shifter <= {1'b1, data, 1'b0};
            nbits   <= 4'd0;
            clkcnt  <= 13'd0;
            busy    <= 1'b1;
        end
    end
endmodule
