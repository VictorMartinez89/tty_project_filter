...
module top (
    ...
    // sincronismo por VSYNC (estable). vblink cuenta cuadros -> diagnostico LED.
    reg href_d = 1'b0, vsync_d = 1'b0;
    reg [4:0]  vblink  = 5'd0;    // cuenta flancos de VSYNC (para el LED rojo)
    reg        parity  = 1'b0;
    reg [7:0]  curY    = 8'd0;
    ...
    always @(posedge cam_pclk) begin
        ...
        vsync_d <= cam_vsync;
        we      <= 1'b0;
        if (cam_vsync & ~vsync_d) begin              // VSYNC sube = nuevo cuadro
            waddr_wr <= 13'd0; rowkeep <= 3'd0;
            vblink   <= vblink + 1'b1;               // diagnostico
        end
        if (~cam_href) begin
    ...
    // sincronizar el bit de diagnostico VSYNC al dominio clk
    reg vbl_s0 = 1'b0, vbl_s1 = 1'b0;
    always @(posedge clk) begin vbl_s0 <= vblink[4]; vbl_s1 <= vbl_s0; end
    ...
    // OFFSET de encuadre: con VSYNC deberia quedar centrado (0). Si queda
    // corrido, ajustar de a ~600. Cada 60 = una fila.
    localparam [13:0] OFFSET = 14'd0;
    ...
    SB_RGBA_DRV #(
        ...
        .RGB0PWM(cfg_done), .RGB1PWM(vbl_s1), .RGB2PWM(hb[23]),
