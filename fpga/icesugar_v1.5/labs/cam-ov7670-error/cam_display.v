// cam_display.v — Paso 3c: CAMARA -> frame buffer (BRAM) -> PANTALLA ILI9341.
// La camara (config grises) captura, se submuestrea a 60x80, se guarda en un
// frame buffer de doble puerto (escribe en dominio pclk, lee en dominio clk),
// y la pantalla lo muestra a 240x320 (escala x4).  = ¡la camara en la pantalla!
//   VERDE = camara configurada (cfg_done), AZUL = heartbeat.
// Pines: clk=35, cam_xclk=2, cam_scl=26, cam_sda=27, cam_pclk=28, cam_href=32,
//        cam_vsync=4, cam_d[0..7]=48,46,44,43,38,34,31,42,
...
module top (
    ...
    // ============ captura camara -> escribe frame buffer (dominio pclk) ============
    // submuestreo: 640->60 columnas (1 de cada 10, recorta), 480->80 filas (1 de cada 6)
    reg href_d=0, vsync_d=0;
    reg        parity  = 1'b0;
    reg [7:0]  curY    = 8'd0;
    reg [3:0]  colkeep = 4'd0;   // 0..9
    reg [2:0]  rowkeep = 3'd0;   // 0..5
    reg [6:0]  fbx     = 7'd0;   // 0..59 columna en fila
    reg [12:0] waddr_wr= 13'd0;  // puntero de llenado 0..4799
    reg        we      = 1'b0;
    reg [12:0] wadr    = 13'd0;
    reg [7:0]  wdat    = 8'd0;
    ...
    always @(posedge cam_pclk) begin
        ...
        vsync_d <= cam_vsync;
        we      <= 1'b0;
        if (cam_vsync & ~vsync_d) begin              // nuevo frame
            waddr_wr <= 13'd0; rowkeep <= 3'd0;
        end
        if (~cam_href) begin                          // inicio de fila / blanking
            parity <= 1'b0; colkeep <= 4'd0; fbx <= 7'd0;
        end else begin                                // pixeles activos
            if (parity == 1'b0) curY <= cam_d;        // luma (byte par)
            else begin
                if (rowkeep==3'd0 && colkeep==4'd0 && fbx<7'd60 && waddr_wr<13'd4800) begin
                    we <= 1'b1; wadr <= waddr_wr; wdat <= curY;
                    waddr_wr <= waddr_wr + 1'b1; fbx <= fbx + 1'b1;
                end
                colkeep <= (colkeep==4'd9) ? 4'd0 : colkeep + 1'b1;
            end
            parity <= ~parity;
        end
        if (href_d & ~cam_href)                       // fin de fila
    ...
    reg [7:0] fb [0:4799];
    reg [7:0] fb_rd = 8'd0;
    always @(posedge cam_pclk) if (we) fb[wadr] <= wdat;   // puerto escritura (camara)
    ...
    reg [7:0] xcol = 8'd0;
    reg [8:0] ycol = 9'd0;
    wire [6:0] fx = xcol[7:2];            // /4
    wire [6:0] fy = ycol[8:2];            // /4
    wire [12:0] raddr = fy*60 + fx;
    always @(posedge clk) fb_rd <= fb[raddr];   // puerto lectura (display)
