// cam_config3.v — Config MINIMA OV7670: solo grises (YUV), SIN escala de resolucion.
// Aisla el problema: si esto abre el rango de brillo, el formato grises anda y el
// PCLK sigue vivo (la resolucion QQVGA se agrega despues, por separado).
//
//   ROJO fijo  = configurando
//   VERDE fijo = config termino
//   AZUL late  = FPGA viva
//   UART: brillo hex. En YUV, hasta en oscuridad deberia leer ~48 (croma=0x80);
//         con luz debe subir. Si lee 00 -> PCLK frenado.
//
// Tabla minima:  COM7=0x80 (reset) -> pausa -> COM7=0x00 (YUV/grises). Fin.
//
...
module top (
    ...
    // ---- tabla MINIMA: solo formato grises, sin escala ----
    reg [15:0] rom;
    reg [4:0]  idx = 5'd0;
    always @(*) case (idx)
        5'd0:  rom = 16'h12_80;   // COM7 RESET
        5'd1:  rom = 16'h12_00;   // COM7 = YUV (luma = grises), VGA por defecto
