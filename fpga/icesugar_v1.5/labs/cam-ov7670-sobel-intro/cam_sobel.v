// cam_sobel.v — Paso 5b: SOBEL 3x3 completo sobre el stream de la camara.
// 2 line buffers (BRAM) -> 3 filas -> ventana 3x3 -> Gx, Gy, |Gx|+|Gy|.
//
// Kernels Sobel:
//   Gx = [-1 0 1; -2 0 2; -1 0 1]   Gy = [-1 -2 -1; 0 0 0; 1 2 1]
//   ventana:  a b c / d e f / g h i    (a=arriba-izq, i=abajo-der)
//   Gx = (c+2f+i) - (a+2d+g)   Gy = (g+2h+i) - (a+2b+c)   mag=|Gx|+|Gy|
//
// UART cada ~0.35s:  "<YM> <GM>"  YM=pico luma, GM=pico de MAGNITUD Sobel.
//   -> apunta a bordes/lineas: GM sube fuerte.  liso: GM bajo.
//   VERDE = fuerza de bordes Sobel, ROJO=configurando, AZUL=heartbeat.
...
module top (
    ...
    // buf1 = fila anterior (n-1) ; buf2 = fila n-2
    reg [7:0] buf1 [0:1023];
    reg [7:0] buf2 [0:1023];
    reg [7:0] rd1 = 8'd0, rd2 = 8'd0;
    always @(posedge cam_pclk) begin
        rd1 <= buf1[col];
        if (wr_en) buf1[col] <= cur;        // fila n -> pasa a ser n-1
    end
    always @(posedge cam_pclk) begin
        rd2 <= buf2[col];
        if (wr_en) buf2[col] <= rd1;        // vieja n-1 -> pasa a ser n-2
    end

    // ================= ventana 3x3 (taps) =================
    // fila 0 = n-2 (rd2), fila 1 = n-1 (rd1), fila 2 = n (cur)
    // tap0 = columna actual c, tap1 = c-1, tap2 = c-2
    reg [7:0] r0t0=0,r0t1=0,r0t2=0;
    reg [7:0] r1t0=0,r1t1=0,r1t2=0;
    reg [7:0] r2t0=0,r2t1=0,r2t2=0;

    // Sobel combinacional (mapa: a b c / d e f / g h i)
    wire [7:0] a=r0t2, b=r0t1, cc=r0t0;
    wire [7:0] d=r1t2,          f=r1t0;
    wire [7:0] g=r2t2, h=r2t1, ii=r2t0;
    wire [10:0] gxp = cc + (f<<1) + ii;      // c + 2f + i
    wire [10:0] gxn = a  + (d<<1) + g;        // a + 2d + g
    wire [10:0] gyp = g  + (h<<1) + ii;       // g + 2h + i
    wire [10:0] gyn = a  + (b<<1) + cc;       // a + 2b + c
    ...
    always @(posedge cam_pclk) begin
        if (!cam_href) begin
            ...
            if (parity == 1'b0) begin
                ...
                // desplazar ventana con las 3 filas nuevas en columna c
                r0t2<=r0t1; r0t1<=r0t0; r0t0<=rd2;
                r1t2<=r1t1; r1t1<=r1t0; r1t0<=rd1;
                r2t2<=r2t1; r2t1<=r2t0; r2t0<=cur;
                // picos
                if (clr != clr_p) begin ymax<=cur; gmax<=mag; clr_p<=clr; end
                else begin
