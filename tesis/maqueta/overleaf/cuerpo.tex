...
\subsection{Cómo se instaló cada
herramienta}\label{cuxf3mo-se-instaluxf3-cada-herramienta}

El flujo completo son doce piezas de software, y la parte laboriosa no
es usarlas sino ponerlas de acuerdo. La documentación de partida fueron
dos repositorios del propio grupo: el manual de instalación del flujo
ASIC mantenido por J. Ruiz (ref. 48) y las notas de la asignatura de
VLSI del director de este trabajo (ref. 49). Lo que sigue describe el
procedimiento y, sobre todo, las decisiones que hubo que tomar cuando
esa documentación no encajaba con la máquina disponible.

Conviene ver las doce piezas no como una lista sino como \textbf{cuatro
maneras distintas de conseguir una herramienta}, porque cada una falla
de forma distinta y cuesta un tiempo distinto.

\textbf{Primera vía: pedírsela al gestor de paquetes.} Es el caso de
\textbf{Icarus Verilog}, el compilador de Verilog con el que se simula
todo el RTL de este trabajo, y de \textbf{GTKWave}, el visor de formas
de onda que lee los \texttt{.vcd} que Icarus produce. Ambos se instalan
con una sola orden del gestor de paquetes de la distribución y no dan
problemas. \textbf{KLayout}, el visor de planos con el que se tomaron
las capturas de los capítulos de resultados, entra por la misma vía. Son
las tres únicas piezas del flujo que se resuelven en un minuto, y no es
casualidad: son también las tres que no tienen que entenderse con el
PDK.

\textbf{Segunda vía: compilarla desde el código fuente.} Aquí están las
cuatro herramientas que forman el núcleo del trabajo. \textbf{Yosys} es
el sintetizador, el programa que convierte el Verilog en una red de
puertas lógicas; se clona su repositorio, se instalan una docena larga
de dependencias de desarrollo ---el compilador, el analizador
sintáctico, las bibliotecas de Tcl, de Boost y de lectura de línea--- y
se compila. \textbf{ngspice} es el simulador SPICE, y su compilación
pide explícitamente el soporte gráfico de X y la biblioteca de línea de
órdenes; se descarga como archivo comprimido en lugar de clonarse, se
configura en un directorio aparte y se compila allí. \textbf{OpenSTA} es
el analizador de tiempos estático, el que dice si un circuito cierra a
la frecuencia pedida; se construye con CMake y necesita además el
generador de interfaces SWIG. \textbf{Magic} es la herramienta de diseño
físico con la que se inspeccionan y extraen los planos, y es la más
exigente de las cuatro: antes de compilarla hay que instalar por
separado el preprocesador de macros, un intérprete de órdenes
alternativo, las cabeceras de X11, las de Tcl y Tk, la biblioteca de
dibujo vectorial, la de OpenGL y la de manejo de terminal.

\begin{quote}
El patrón es constante: \textbf{en estas cuatro herramientas el tiempo
no se va en compilar, se va en descubrir qué dependencia falta.} El
mensaje de error de una dependencia ausente aparece a mitad de la
compilación y nombra un fichero de cabecera, no un paquete, de modo que
hay que traducir del nombre del fichero al nombre del paquete. Instalar
la lista completa de dependencias antes de empezar, aunque parezca
excesivo, sale más barato que descubrirlas de una en una.
\end{quote}

\textbf{Tercera vía: traerla dentro de un contenedor.} \textbf{OpenLane}
es el flujo automatizado que lleva el RTL hasta el GDSII, y no es un
programa sino una orquestación: por dentro invoca a OpenROAD para el
emplazamiento y el ruteo, a Yosys para la síntesis, a Magic y
\textbf{Netgen} para la verificación física y a KLayout para las
comprobaciones geométricas. Instalar esas piezas a mano y en versiones
compatibles entre sí es precisamente el problema que el contenedor
resuelve, y por eso OpenLane se distribuye como imagen de
\textbf{Docker}. Docker, a su vez, no se instala desde el repositorio de
la distribución sino desde el del propio proyecto, lo que obliga a
registrar su clave de firma y su fuente de paquetes antes de poder
pedirlo. Hecho eso, la instalación de OpenLane se reduce a clonar el
repositorio y lanzar su compilación, que en realidad descarga la imagen
y comprueba que el contenedor arranca. \textbf{Netgen y OpenROAD no se
instalan nunca por separado en este trabajo: viven dentro de esa
imagen}, y de hecho no aparecen como órdenes disponibles en la máquina
anfitriona.

\textbf{Cuarta vía: la que no es una herramienta sino una biblioteca.}
El \textbf{PDK} ---el conjunto de reglas, modelos y celdas que describe
el proceso de fabricación--- es lo que convierte a todas las anteriores
en algo capaz de producir un circuito real. Se obtiene con
\textbf{open\_pdks}, que se clona, se configura pidiéndole
explícitamente el proceso \textbf{sky130} y se compila; el resultado es
el árbol \texttt{sky130A} que todas las demás herramientas consultan.
Es, con diferencia, la instalación más voluminosa de todo el entorno:
ocupa alrededor de veintiséis gigabytes de disco, más de lo que ocupan
juntas todas las herramientas del flujo. En la práctica se acabó usando
además un gestor de versiones de PDK, que descarga una compilación ya
hecha de unos dos gigabytes y permite fijar exactamente qué versión del
proceso se usó en cada circuito; esa trazabilidad importa, porque un
cambio de versión del PDK cambia las cifras de área y de tiempos.

Quedan dos piezas fuera de esas cuatro vías. \textbf{Xyce}, el simulador
de circuitos analógicos de alto rendimiento con el que se hizo la
verificación eléctrica del camino crítico del capítulo 5, se compila
mediante un guion de construcción publicado por terceros que resuelve
por dentro sus dependencias. Y el \textbf{entorno de Tiny Tapeout},
necesario para reproducir exactamente el circuito enviado a fabricación,
no se instala: se reconstruye con un guion que fija las versiones
históricas del proyecto ---una versión concreta del flujo, una del PDK y
una de las herramientas de soporte--- porque el objetivo allí no es
tener lo más nuevo sino tener lo mismo que había el día del envío.

\subsection{El punto en que la documentación heredada deja de
servir}\label{el-punto-en-que-la-documentaciuxf3n-heredada-deja-de-servir}

Los dos manuales de partida suponen una máquina con procesador x86-64.
Este trabajo se hizo sobre \textbf{ARM de 64 bits}, y ahí la instalación
deja de ser una transcripción.

La diferencia aparece literalmente escrita en la orden que registra el
repositorio de Docker, que declara la arquitectura \texttt{amd64}, y
vuelve a aparecer en la imagen de OpenLane, que hubo que tomar en su
variante \texttt{arm64v8}. Las herramientas que se compilan desde el
código fuente cruzan sin problema ---se compilan para la máquina en la
que están---, pero \textbf{todo lo que se distribuye ya compilado hay
que buscarlo en la arquitectura correcta, y no siempre existe}. Ésa es
la razón de fondo por la que este trabajo se reparte entre dos máquinas
y no una: no es una preferencia, es que ciertas piezas del flujo sólo
funcionan donde funcionan.

\begin{quote}
La lección, que vale más allá de este proyecto: \textbf{un manual de
instalación no es un procedimiento, es el registro de un procedimiento
que funcionó en una máquina concreta.} Reproducirlo exige identificar
antes qué de lo que dice depende de esa máquina.
\end{quote}

Una segunda asimetría, menor pero constante, es que una parte del flujo
se resolvió con una \textbf{distribución empaquetada} que trae ya
compilados el sintetizador, el emplazador para FPGA, el compilador de
Verilog, el visor de ondas y las utilidades de programación de la
tarjeta. Conviene decirlo con claridad porque afecta a la
reproducibilidad: \textbf{algunas herramientas de este trabajo están
instaladas dos veces, en dos versiones distintas, en dos máquinas
distintas}, y los resultados de FPGA salen de una de ellas y los de ASIC
de la otra.

\subsection{Las versiones con las que se obtuvieron los
resultados}\label{las-versiones-con-las-que-se-obtuvieron-los-resultados}

Una instalación no queda descrita hasta que se dice qué versión quedó
instalada. Las cifras de este documento se obtuvieron con estas:
...
Icarus Verilog, GTKWave & Distribución empaquetada & Ambas \\
Magic & 8.3.656 & Máquina virtual \\
OpenLane & v1.0.2 & Contenedor, variante ARM \\
Docker & 29.5.3 & Máquina virtual \\
PDK & sky130A & Gestor de versiones de PDK \\
Sistema operativo & Ubuntu sobre ARM de 64 bits & Máquina virtual \\
\end{longtable}

El anexo A recoge el reparto de trabajo entre las dos máquinas y los
errores que esa frontera introduce.
