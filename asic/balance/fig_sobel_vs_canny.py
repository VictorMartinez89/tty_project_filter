...
#   Seis parejas Sobel/Canny con GDS limpio (DRC/LVS/XOR = 0).
#   Datos: recuento HOMOGENEO post-emplazamiento, contado sobre los netlists .nl.v
#   archivados (asic/recuento/contar_celdas.py, 21-sep-2026). NO mezclar con celdas
#   de sintesis: el emplazamiento agrega entre 16 % y 26 %, factor medio x1,21.
import numpy as np
...
P = [("el filtro, solo",                       5823,   12993,  .167,  .360, True),
     ("+ CPU FemtoRV32 (SoC)",                12043,   22054,  .370,  .670, True),
     ("cámara + framebuffer + display",       35653,   41925, 1.750, 2.040, True),
     ("MNIST en IHP · Tiny Tapeout",          13319,   14970,  .541,  .541, True),
     ("cámara + CPU + clasificador",          19949,   20921,  .845,  .890, True),
     ("cámara + clasificador + LCD",          38643,   39794, 2.032, 2.092, True)]
...
for i, v in enumerate([972, 6220]):
    ax.text(i, v + 150, "%d celdas" % v, ha="center", fontsize=10, weight="bold",
            color=[CANNY, SOBEL][i])
ax.set_ylim(0, 7600); ax.set_ylabel("celdas estándar")
ax.set_title("E · Seis veces más barato\n"
