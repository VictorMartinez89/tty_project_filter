...
module FemtoRV32(
 ...
 // Latched instruction (declarada aqui arriba para iverilog: se usa antes de la
 // seccion de PC). Bits 0 y 1 se ignoran (no se usan en RV32I base).
 reg  [31:2] instr;

 // --- Declaraciones ADELANTADAS para iverilog (redes usadas antes de su assign).
 //     La red se declara aqui; su 'assign' queda mas abajo donde ya existen sus operandos. ---
 wire        funct3IsShift;
 ...
 /* verilator lint_on UNUSED */
   ...
   // (reg [31:2] instr;  <- movida arriba para iverilog)
   ...
   // (reg writeBack; <- movida arriba para iverilog)
...
//        mem_wmask <= 0;
      ...
      // default: evita el LATCH (y el lazo combinacional que nextpnr rechaza) cuando
      // 'state' no es un one-hot valido. No cambia el comportamiento (esos estados no ocurren).
      default: begin
        mem_rstrb = 0;
        aluWr     <= 0;
        writeBack = 0;
...
/* verilator lint_on WIDTH */
   ...
   // (reg cycles; <- movida arriba para iverilog)
